...
export default class ImageMarker {
  ...
  addImageView() {
    ...
    let basePath = getBasePath(this.editor);
    let linkURL = parseImageLocation(result[1], basePath);
...
function parseImageLocation(text, basePath) {
  let imagePath = text.trim();
  // try to parse selected text as md link
  let mdLink = parseMarkdownLink(imagePath);
  if (mdLink) {
...
function checkFile(filePath, onErr) {
  try {
