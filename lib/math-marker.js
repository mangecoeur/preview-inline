...
export default class MathMarker {
  constructor(cursor, editor) {
    ...
    let result = this.getMathAroundCursor(cursor);
    if (!result) {
  ...
  getMathInline(scopeString) {
    let range = this.editor.bufferRangeForScopeAtCursor(scopeString);
    let text = this.editor.getBuffer().getTextInRange(range);
  ...
  getMathBlock(scopeString) {
    ...
    while (scopeTools.scopeContains(curScope, scopeString)) {
      minRow -= 1;
      curPos = [minRow, 0];
      curScope = this.editor.scopeDescriptorForBufferPosition(curPos);
      // var line = this.editor.lineTextForBufferRow(minRow);
    }
  ...
  getMathAroundCursor(cursor) {
    ...
    if (scopeString) {
      return this.getMathInline(scopeString);
    } else {
      scopeString = scopeTools.scopeContainsOne(scope, mathBlockScopes);
      if (scopeString) {
        return this.getMathBlock(scopeString);
      } else {
        return null;
      }
    }
